	...
	public void keyPressed(KeyEvent arg0) {
		...
		case KeyEvent.VK_UP :
			...
			if(gui.getHistoryPointer() < 0) {
				...
				gui.commandField.setText(gui.getCommandHistory().get(gui.getHistoryPointer()));
			}
			break;
			...
			if(words.length == 2) {
				if(words[0].equalsIgnoreCase("edit")) {
					try {
						...
						if(id > 0 && id <= gui.getLogic().getTaskListFromLabel(gui.getLogic().getCurrLabel()).size()) {
							Task task = gui.getLogic().getTaskListFromLabel(gui.getLogic().getCurrLabel()).get(id-1);
							TDTDateAndTime dat = task.getDateAndTime();
							String datString = gui.getDateTimeStringForEdit(dat);
							gui.commandField.setText(gui.getUserCommand() + " " +task.getDetails() + datString);
							javax.swing.SwingUtilities.invokeLater(new Runnable() {
								   public void run() { 
									   gui.commandField.select(gui.getUserCommand().length() + 1,
											   gui.commandField.getText().length() - 1);
								   }
								});
						}
				...
				if(words[0].equalsIgnoreCase("edit")) {
					try {
						...
						if(gui.getLogic().isInLabelMap(words[1].toUpperCase())) {
							if(id > 0 && id <= gui.getLogic().getTaskListFromLabel(words[1]).size()) {
								Task task = gui.getLogic().getTaskListFromLabel(words[1]).get(id-1);
								TDTDateAndTime dat = task.getDateAndTime();
								String datString = gui.getDateTimeStringForEdit(dat);
								gui.commandField.setText(gui.getUserCommand() + " " + task.getDetails() + datString);
								javax.swing.SwingUtilities.invokeLater(new Runnable() {
